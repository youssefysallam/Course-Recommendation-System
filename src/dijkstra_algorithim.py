...
def dijkstra(courses, start):
  priorty_queue = []
  distances = {course: float('inf') for course in courses} #Initializes all distances to infinity except the start node, which is srt to 0.
  distances[start] = 0
  previous_nodes = {course: None for node in courses}
  heapq.heappush(priorty_queue, (0, start)) #Adds start node initialized to distance = 0.
  ...
  while priorty_queue:
    ...
    for prereq_group in courses[current_course].prerequisites: #Processes all prereq groups for current selected course.
      for prereq_course in prereq_group.courses:
        ...
        if distance < distances[prereq_course.code]:
          ...
          previous_nodes[prereq_course.code] = current_courseheapq.heappush(priorty_queue, (distance, prereq_course.code))
    return distances, previous_nodes
  
def recommend_courses(course, completed_courses): # Here the function is used to suggest cources that can be taken next based on completed prereqs.
  recommend = []
  ...
  def main():
    ...
    parser.add_argument("--major", type=str, required=True, help="The major to load requirements for.")
    parser.add_argument("--completed", type=str, nargs="*", default=[], help="List of completed courses.")
    ...
    courses = {course_code: Course(course_code, data["credits"]) for course_code, data in graph.items()}
    for course_code, data in graph.items():
      for prereq_group in data["prerequisites"]:
        courses[course_code].prerequisites.append(
          DependencyGraph(prereq_group["count"], [courses[prereq] for prereq in prereq_group["courses"]])
        )
